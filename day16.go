...
func solve16(grid []string, start state) *solver {
...
func part1Day16(lines []string) string {
	...
	s := solve16(lines, start)
	s.printgrid()
	return fmt.Sprintf("%d", s.cheapest)
}

func part2Day16(lines []string) string {
	start := state{pos: position{row: len(lines) - 2, col: 1}, dir: east}
	if lines[start.pos.row][start.pos.col] != 'S' {
		start = state{pos: position{row: 1, col: len(lines[0]) - 2}, dir: south}
	}
...
func day16Calculate() {
	dat, err := os.ReadFile("inputs/day16.txt")
	if err != nil {
		log.Fatal(err)
	}
	lines := strings.Split(string(dat), "\n")
	fmt.Println(part1Day16(lines))
	fmt.Println(part2Day16(lines))
}
