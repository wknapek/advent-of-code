...
func day18Calculate() {
	dat, err := os.ReadFile("inputs/day18.txt")
	if err != nil {
		log.Fatal(err)
	}
	fmt.Println(day18(string(dat), 1))
	fmt.Println(day18(string(dat), 2))
}
...
func parseInput18(input string) []Coord {
	var coords []Coord
	for _, line := range strings.Split(input, "\n") {
		n := GetIntsInString(line)
